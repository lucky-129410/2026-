...
### 多人协作
git remote -v || git remote查看远程仓库信息 默认名称为origin

git push origin master-->将当前分支推送到远程

![alt text](image-4.png)



### 标签管理 tag-->跟commit绑定在一起
#### 创建tag
先切换到要打标签的分支上
git tag <name>--> git tag v1.0 默认标签是打在最新提交的commit上的
git tag -->查看所有提交
git show <name>查看说明
若果忘了打标签 先用git log --pretty=oneline --abbrev-commit查找要打标签的commit-id 然后用$ git tag commit-id
用-a指定标签名，-m指定说明文字：
